...
class AlphaBottleneckDeprecated(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(AlphaBottleneckDeprecated, self).__init__()
...
class AlphaBlockDeprecated(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(AlphaBlockDeprecated, self).__init__()
...
class AlphaNetDeprecated(ResNet):
    def __init__(self, game, layers):
        block = AlphaBlockDeprecated
        ...
        self.conv1 = nn.Conv2d(18, 128, kernel_size=5, stride=1, padding=2,
                               bias=False)
    ...
    def forward(self, x):
        # print("forward")
        x = x.view(-1, 18, self.board_x, self.board_y)
...
class AlphaNetMakerDeprecated:
    ...
    def resnet18(self, game, pretrained=False):
        ...
        """
        model = AlphaNetDeprecated(game, [2, 2, 2, 2])
        if pretrained:
    ...
    def resnet34(self, game, pretrained=False, **kwargs):
        ...
        Args:
            pretrained (bool): If True, returns a model pre-trained on ImageNet
        """
        model = AlphaNetDeprecated(game, [3, 4, 6, 3])
        if pretrained:
